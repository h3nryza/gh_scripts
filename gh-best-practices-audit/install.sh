#!/usr/bin/env bash
# install.sh — Remote installer for gh-best-practices-audit
# Written by h3nryza
#
# Usage (remote):
...
readonly REPO_RAW="https://raw.githubusercontent.com/h3nryza/gh_scripts/main/gh-best-practices-audit"
readonly SCRIPT_NAME="gh-best-practices-audit"
readonly MAIN_SCRIPT="gh_best_practices_audit.py"
readonly LAMBDA_SCRIPT="lambda_handler.py"
readonly REQUIREMENTS="requirements.txt"
readonly VERSION="1.0.0"

# Rules files
readonly -a RULES_FILES=(
    "cis_benchmark.json"
    "owasp_cicd.json"
    "sans_top25.json"
    "custom_template.csv"
...
show_banner() {
    echo -e "${BOLD}${CYAN}"
    echo "╔══════════════════════════════════════════════════╗"
    echo "║  gh-best-practices-audit Installer — h3nryza    ║"
    echo "╚══════════════════════════════════════════════════╝"
    echo -e "${RESET}"
}
...
show_help() {
    cat <<EOF
gh-best-practices-audit installer — Written by h3nryza

USAGE:
  ./install.sh [OPTIONS]

OPTIONS:
  --prefix <dir>    Installation directory (default: ~/.local/bin/gh-best-practices-audit)
...
download_all() {
    ...
    for f in "${RULES_FILES[@]}"; do
        download_file "${REPO_RAW}/rules/${f}" "${INSTALL_DIR}/rules/${f}"
        log_success "Downloaded: rules/${f}"
    done
}
...
install_local() {
    ...
    mkdir -p "${INSTALL_DIR}/rules"
    for f in "${RULES_FILES[@]}"; do
        [[ -f "${src_dir}/rules/${f}" ]] || { log_warn "Skipping missing rules file: ${f}"; continue; }
        cp "${src_dir}/rules/${f}" "${INSTALL_DIR}/rules/${f}"
        log_success "Copied: rules/${f}"
    done
}
...
create_wrapper() {
    ...
    cat > "${wrapper}" <<WRAPPER
#!/usr/bin/env bash
# Auto-generated wrapper for gh-best-practices-audit
...
add_to_path_hint() {
    ...
    if [[ -n "${shell_rc}" ]]; then
        if ! grep -q "${INSTALL_DIR}" "${shell_rc}" 2>/dev/null; then
            {
                echo ""
                echo "# Added by gh-best-practices-audit installer"
...
main() {
    ...
    echo "  Rules directory:  ${INSTALL_DIR}/rules/"
    echo ""
